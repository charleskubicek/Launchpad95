...
class CKNoteSelectorComponent(ControlSurfaceComponent):
    ...
    def set_enabled(self, enabled):
        ...
        self._buttons[0].send_value("CKSeq.LightBlue")
        self._buttons[1].send_value("CKSeq.Blue")
        self._buttons[1].send_value("CKSeq.LightBlue")
        self._buttons[2].send_value("CKSeq.Blue")
        self._buttons[3].send_value("CKSeq.LightBlue")
        self._buttons[4].send_value("CKSeq.Blue")
        self._buttons[5].send_value("CKSeq.LightBlue")
        self._buttons[6].send_value("CKSeq.Blue")
        self._buttons[7].send_value("CKSeq.LightBlue")
        self._buttons[8].send_value("CKSeq.Blue")
        self._buttons[9].send_value("CKSeq.LightBlue")

        self._buttons[10].send_value("CKSeq.Off")
        self._buttons[11].send_value("CKSeq.Off")
        self._buttons[12].send_value("CKSeq.Off")
        self._buttons[13].send_value("CKSeq.Off")
        self._buttons[14].send_value("CKSeq.Off")
        self._buttons[15].send_value("CKSeq.Off")

        self._buttons[self._shift_button_index].send_value("CKSeq.Shift")
...
class CKStepSequencerComponent(CompoundComponent):
    ...
    def on_scene_trigger(self, value, sender):
        if value != 0:
            ...
            self._control_surface.log_message(f"on_scene_trigger sender = {sender}")
            self._control_surface.log_message(f"on_scene_trigger sender name = {sender.name}")
            self._control_surface.log_message(f"on_scene_trigger sender.identifier = {sender.identifier}")

            # idx = int((sender.identifier / 8) / 2)
            idx = int(sender.identifier )
            self._control_surface.log_message(f"on_scene_trigger idx = {idx}")
    ...
    def _set_note_selector(self):
        ...
        self._note_selector = self.register_component(CKNoteSelectorComponent(self, pads, self._control_surface))
    ...
    def _on_notes_changed(self):  # trigger by callback on clip or via _clip_changed.
        self._control_surface.log_message("Notes changed")
        if self.is_enabled():
            self._note_editor.update()
